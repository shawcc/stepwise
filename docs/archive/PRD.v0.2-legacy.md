...
### 4.2 Objective 与 Key Results

每个 Goal Node 使用 OKR 作为标准目标协议：

- Objective 表达希望发生的变化、方向和价值。
- Key Results 定义怎样判断 Objective 已经达成。
- KR 必须在执行前确定，描述结果而不是任务，并且可衡量或可验证。
- 每个 KR 必须连接口径、时间范围、证据要求和最终验收结论。

例如：

```text
Objective：让新客户更快获得产品价值
KR1：新客户激活率从 32% 提升到 50%
KR2：首次价值时间从 14 天缩短到 7 天
```
...
### 4.7 必要条件 Necessary Condition

必要条件是目标成立不可缺少的条件，是待验证的因果判断。判断方法是：去掉这个条件，目标是否仍能成立？如果不能，它才可能是必要条件。

必要条件与 KR 必须严格区分：

- KR 属于测量侧，回答“看到什么结果才算成功”。
- 必要条件属于原因侧，回答“缺少什么会使目标无法成立”。
- 条件全部满足不保证 KR 一定达成，因此整组条件只能作为联合充分假设，必须接受执行证据检验。
- 如果某个条件缺失但 KR 仍达成，说明该条件并非必要条件，相关推理需要修正。
...
WorkGraph 采用 OKR 作为目标层的标准协议，并以图谱补全推理、执行、证据和治理：

| OKR 概念 | WorkGraph 对应概念 | 差异 |
|----------|-------------------|------|
| Objective | Goal Node 的目标表达 | Goal 还包含来源、责任、条件、约束和推理记录。 |
| Key Results | Goal Node 的正式成功定义 | 每个 KR 连接口径、证据和验收结论。 |
| Initiative | 路线、子 Goal 与 Action | 子 Goal 到下一级负责人处成为新的 Objective。 |
| Check-in | 执行更新与 Review | Review 会区分执行偏差和因果假设错误，并可触发 Redo。 |

产品定位是“OKR 入口 + WorkGraph 内核”：Objective 与 KR 定义目标和成功，必要条件解释原因，路线创造条件，执行产生证据，验收与复盘校准 KR、条件和路线。
